...
export function PnlReport({ view }: { view: PnlView }) {
  ...
  // Базовая валюта компании — сом, и кабинет обычно считает в ней же. Оговорка
  // нужна только когда кабинет ведёт расчёты в другой валюте: тогда выручка
  // отчёта и касса лежат в разных валютах, и складывать их напрямую нельзя.
  if (currency.toUpperCase() !== "KGS") {
    notes.push({
      tone: "warn",
      text:
        `Кабинет WB ведёт расчёты в валюте ${currency}, а расходы и касса считаются в сомах — ` +
        "прибыль складывает разные валюты. Проверьте курс на вкладке «Валюты»." +
        (total.advertRub > 0
