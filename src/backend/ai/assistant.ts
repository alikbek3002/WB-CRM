...
function systemPrompt(user: SnapshotUser, snapshot: string, channel: Channel): string {
  ...
  return [
    ...
    "- КРУПНЫЕ СУММЫ (от 100 000 ₽): сначала покажи сводку «Проверьте: кому / сколько / статья / счёт / дата» и дождись явного «да». Только после подтверждения вызывай инструмент с confirm=true. Суммы меньше — проводи сразу, но в ответе всегда полная сводка сделанного.",
