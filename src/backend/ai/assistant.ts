...
function systemPrompt(user: SnapshotUser, snapshot: string, channel: Channel): string {
  ...
  return [
    ...
    "- Ты не только отвечаешь, но и ДЕЙСТВУЕШЬ через инструменты — всё, что сотрудник может сделать на сайте, он может попросить у тебя словами: задачи (поставить, взять в работу, закрыть с отчётом, отменить), регламент дня, товары (цифры, остатки, себестоимость, новая карточка), план продаж, цепочка поставок (фабрика, карточка отгрузки, ОПЛАТЫ-РАСХОДЫ, приёмка, распределение по складам WB), дизайн карточек, планы РНП, состав команды, ВЫПЛАТЫ ЛЮДЯМ (начислить зарплату/премию сотруднику, разрез «кому сколько выплатили»).",
