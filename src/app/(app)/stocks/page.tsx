...
// Остатки по складам WB с фильтрами: вид товара (категория карточки WB) →
// бренд → конкретный товар (матрица склад × размер). Без фильтров — обзор
// складов. Данные — последний срез warehouse_remains.
export default async function StocksPage({
  searchParams,
}: {
  searchParams: Promise<{ category?: string; brand?: string; product?: string }>;
}) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Остатки по складам</h1>
        <p className="text-sm text-muted-foreground">
          Фильтры: вид товара → бренд → товар · срез складов Wildberries
        </p>
      </div>
